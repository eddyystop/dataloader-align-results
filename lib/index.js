...
module.exports = function dataLoaderAlignResults (options = {}) {
  ...
  const { graphqlType, serializeRecordKey,
    serializeDataLoaderKey = key => key.toString(), onError = () => {}, isStrict } = options;
  ...
  const defaultElem = isStrict ? null : [];
  ...
  return function dataLoaderAlignResultsInner (keys, resultArray) {
    debug('started', graphqlType, keys, resultArray);
    ...
    const result = keys.map((key, i) => {
      ...
      const value = hash[serializedKey] || defaultElem;
